...
export default function Students() {
  ...
  const handleDeleteStudent = (id: string | number) => deleteStudentMutate.mutate(id)

  const totalStudentsCount = Number(queryStudents.data?.headers['x-total-count'] || 0)
  const totalPage = Math.ceil(totalStudentsCount / LIMIT)

  const handlePrefetchStudent = (id: number) => {
    queryClient.prefetchQuery(['students', String(id)], {
        ...
        <Fragment>
          <div className='relative mt-6 overflow-x-auto shadow-md sm:rounded-lg'>
            <table className='w-full text-left text-sm text-gray-500 dark:text-gray-400'>
              ...
              <tbody>
                {queryStudents.data?.data.map((student, _index) => (
                  <tr
                    key={student.id}
          ...
          <div className='mt-6 flex justify-center'>
            <nav aria-label='Page navigation example'>
              <ul className='inline-flex -space-x-px'>
                ...
                {page === totalPage ? (
                  <li className='rounded-r-lg border border-gray-300 bg-white py-2 px-3 leading-tight text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'>
                    Next
                  </li>
                ) : (
                  <li>
